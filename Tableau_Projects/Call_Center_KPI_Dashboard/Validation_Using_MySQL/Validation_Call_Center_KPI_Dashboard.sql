-- preview full raw dataset
SELECT * 
FROM call_center_kpi_data.call_center_data;


-- validate call volume by weekday for the latest available month
SELECT 
...
-- validate satisfaction rating distribution for the latest month
...
-- validate inbound calls for the latest available date
SELECT COUNT(call_id) AS inbound_calls_today
FROM call_center_data
WHERE `Date` = (SELECT MAX(`Date`) FROM call_center_data);


-- validate overall resolution rate for the latest month
SELECT 
    ROUND(
...
-- validate resolution rate by agent for the latest month
SELECT 
    Agent,
    ROUND(
...
-- validate total resolved calls handled by each agent (latest month)
SELECT 
    Agent,
...
-- validate median speed of answer by agent (resolved calls, latest month)
SELECT 
    t.Agent, 
    ROUND(AVG(t.speed_answer_sec), 2) AS median_speed_answer_sec
FROM (
    SELECT 
        Agent, 
        speed_answer_sec,
